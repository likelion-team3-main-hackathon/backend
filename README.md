...
MCC Wellness 백엔드는 사용자의 온보딩 프로필과 건강 데이터를 바탕으로 AI 건강 분석, 맞춤 식단·운동 루틴 생성, 루틴 수행 기록, 분석실 집계와 크레딧 보상을 제공하는 Spring Boot API 서버입니다. 건강 문서는 OCR 구조화 결과와 원본 파일을 분리해 저장하고, 인바디·진료 기록·눈바디 사진을 종합 AI 분석에 전달합니다. 문서가 없는 사용자는 프로필만으로도 분석 작업을 시작할 수 있으며, 이후 문서를 추가해 분석을 보완할 수 있습니다.

## 핵심 구조

- Spring Boot REST API: 인증, 온보딩, 건강 문서, AI 작업, 루틴, 기록, 분석실
- MySQL + Flyway: 사용자·루틴·기록·분석 이력과 마이그레이션 관리
- Redis: AI 요청 제한과 작업 보조 저장소
- MinIO: 건강 문서·눈바디·인증 사진의 Private Object Storage
- Gemini Adapter: OCR, 건강 종합 분석, 루틴 생성, 식단 사진·운동 자세 분석
- DB 기반 비동기 Job Worker: 재시도, 멱등성, 상태 조회, 사용자별 요청 한도

식단 완료 항목은 건당 10C, 같은 날짜 운동·재활 항목을 모두 완료하면 20C를 지급하며 프로필 API의 `creditBalance`로 조회합니다.
...
- [x] **DB / Model:** Flyway `V1`~`V17`, 단순화 스키마와 AI 요청 제한·일차 요약·문서 추출값/체성분 측정값·운동 부위 필드 구축
...
- [x] **Gemini Adapter:** PDF/JPG/PNG 다중 문서 추출, 인바디·알레르기·진단서·눈바디 종합 분석, 추천 카드 생성, 선택 기반 전체 기간 루틴 생성, 기록 코칭 구현. 키가 없는 기본 환경은 결정적 Fake 사용
