...
public class PoseAnalysisAiClient {
    ...
    public Result analyze(byte[] image, String contentType, String exerciseName) {
        if (!"gemini".equalsIgnoreCase(provider)) {
            return new Result(82, List.of("무릎이 발끝보다 안쪽으로 모여 있어요."),
                    List.of("무릎을 두 번째 발가락 방향으로 밀어주세요."), 0.78, "", model,
        ...
        try {
            ...
            JsonNode result = json.readTree(response.path("candidates").path(0).path("content")
                    .path("parts").path(0).path("text").asText());
            return new Result(
                    Math.max(0, Math.min(100, result.path("poseScore").asInt())),
                    strings(result.path("detectedIssues")), strings(result.path("feedback")),
                    Math.max(0, Math.min(1, result.path("confidence").asDouble())),
                    result.path("safetyWarning").asText(""), model, prompt.storedVersion());
    ...
    private List<String> strings(JsonNode values) {
        ...
        values.forEach(value -> { if (!value.asText().isBlank()) result.add(value.asText()); });
        return result;
    }

    public record Result(int poseScore, List<String> detectedIssues, List<String> feedback,
            double confidence, String safetyWarning, String modelVersion, String promptVersion) {}
}
