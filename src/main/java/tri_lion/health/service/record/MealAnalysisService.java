...
public class MealAnalysisService {
        ...
        try {
            return analyses.save(
                    new MealAnalysis(
                            ...
                            routineItemId,
                            imageKey,
                            json.writeValueAsString(result.foods()),
                            json.writeValueAsString(totals(result.foods())),
                            BigDecimal.valueOf(result.confidence()),
                            result.modelVersion(),
                            Optional.ofNullable(recordedAt)
                                    .orElseGet(OffsetDateTime::now)
                                    .toInstant()));
        } catch (Exception exception) {
            throw new IllegalArgumentException("식단 분석 결과를 저장하지 못했습니다.", exception);
        }
    }

    public MealAnalysis get(Long id) {
        return owned(id);
    }
    ...
    public MealAnalysis update(Long id, MealAnalysisUpdateRequest request) {
        ...
        try {
            List<MealPhotoAiClient.Food> foods =
                    request.foods().stream()
                            .map(
                                    f ->
                                            new MealPhotoAiClient.Food(
                                                    f.name(),
                                                    f.servingGrams(),
                                                    f.calories(),
                                                    f.carbohydrateGrams(),
            ...
            throw new IllegalArgumentException("수정한 식단을 저장하지 못했습니다.", exception);
        }
    }
    ...
    public MealAnalysis confirm(Long id) {
        ...
        try {
            Map<String, Object> totals =
                    json.readValue(analysis.getTotals(), new TypeReference<>() {});
            List<Map<String, Object>> foods =
                    json.readValue(analysis.getFoods(), new TypeReference<>() {});
            Map<String, Object> details = new LinkedHashMap<>(totals);
            details.put("foods", foods);
            details.put("menu", foods.stream().map(f -> f.get("name")).toList());
            details.put("completed", true);
            details.put("source", "MEAL_PHOTO_ANALYSIS");
            ActivityRecord record =
                    replaceCompletedMealRecord(analysis, details)
                            .orElseGet(
                                    () ->
                                            recordService.create(
                                                    new RecordRequest(
                                                            analysis.getRoutineItemId(),
                                                            ActivityType.MEAL,
            ...
            throw new IllegalArgumentException("식단 기록을 확정하지 못했습니다.", exception);
        }
    }
    ...
    public Map<String, Object> daily(LocalDate date) {
        ...
        for (ActivityRecord record :
                records.findByUserIdAndTypeAndPerformedAtBetweenOrderByPerformedAtDescCreatedAtDesc(
                        userId, ActivityType.MEAL, from, to)) {
            try {
                consumed.add(json.readTree(record.getDetails()));
    ...
    private Totals plannedTarget(Long userId, LocalDate date) {
        ...
        routines.findFirstByUserIdAndStartDateLessThanEqualAndEndDateGreaterThanEqualAndDeletedAtIsNullOrderByCreatedAtDesc(
                        userId, date, date)
                .ifPresent(
                        r ->
                                items.findByRoutineIdAndScheduledDateAndItemTypeAndDeletedAtIsNull(
                                                r.getId(), date, "MEAL")
                                        .forEach(
                                                item -> {
                                                    try {
    ...
    private MealAnalysis owned(Long id) {
    ...
    private Map<String, Double> totals(List<MealPhotoAiClient.Food> foods) {
        ...
        foods.forEach(
                f -> {
                    ...
                    total.carbohydrateGrams += f.carbohydrateGrams();
                    total.proteinGrams += f.proteinGrams();
                    total.fatGrams += f.fatGrams();
                });
        return total.asMap();
    }

    private String comment(Totals difference) {
        if (difference.proteinGrams < 0)
            return "오늘 단백질이 목표보다 " + Math.round(-difference.proteinGrams) + "g 부족했어요.";
        if (difference.calories > 0)
            return "오늘 섭취 열량이 목표보다 " + Math.round(difference.calories) + "kcal 많았어요.";
    ...
    public Map<String, Object> view(MealAnalysis a) {
        try {
            return Map.of(
                    ...
                    a.getStatus(),
                    "foods",
                    json.readTree(a.getFoods()),
                    "totals",
                    json.readTree(a.getTotals()),
                    "confidence",
                    a.getConfidence(),
                    "imageKey",
                    a.getImageKey(),
                    ...
                    a.getModelVersion(),
                    "recordId",
                    Optional.ofNullable(a.getConfirmedRecordId()).orElse(0L));
        } catch (Exception e) {
    ...
    public static class Totals {
        ...
        public Totals(double c, double carb, double protein, double fat) {
            calories = c;
        ...
        void add(JsonNode n) {
            ...
            carbohydrateGrams += number(n, "carbohydrateGrams", "carbs");
            proteinGrams += number(n, "proteinGrams", "protein");
            fatGrams += number(n, "fatGrams", "fat");
        }

        Totals minus(Totals other) {
            return new Totals(
        ...
        private double number(JsonNode n, String... keys) {
            for (String k : keys) if (n.has(k)) return n.path(k).asDouble();
            return 0;
        }
    }
}
