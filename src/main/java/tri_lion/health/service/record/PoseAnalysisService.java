...
public class PoseAnalysisService {
    ...
    public PoseAnalysisService(PoseAnalysisRepository analyses, RoutineRepositories.Items items,
            RoutineRepositories.Routines routines, RecordService records, PoseAnalysisAiClient ai,
            ObjectStorage storage, AuthenticatedUser auth, ObjectMapper json) {
    ...
    @Transactional
    public PoseAnalysis create(MultipartFile image, Long routineItemId, String requestedExerciseName) {
        ...
        if (routineItemId != null) {
            var item = items.findById(routineItemId)
        ...
        PoseAnalysisAiClient.Result result = ai.analyze(storage.get(imageKey), image.getContentType(), exerciseName);
        try {
            return analyses.save(new PoseAnalysis(userId, routineItemId, exerciseName, imageKey,
                    result.poseScore(), json.writeValueAsString(result.detectedIssues()),
                    json.writeValueAsString(result.feedback()), BigDecimal.valueOf(result.confidence()),
                    result.safetyWarning(), result.modelVersion(), result.promptVersion()));
    ...
    public Map<String, Object> view(PoseAnalysis analysis) {
        try {
            ...
            result.put("safetyWarning", Optional.ofNullable(analysis.getSafetyWarning()).orElse(""));
